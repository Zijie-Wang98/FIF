%%% calculate the FIF for the entire network
%%% the sensitivity is not considered

function [FIF_theoretical,FIF_contribution,SPEB_pairwise] = calculate_FIF_theoretical(Txlist,Rxlist,target_list)
    Nt = length(Txlist);
    Nr = length(Rxlist);
    FIF = zeros(Nt,Nr);
